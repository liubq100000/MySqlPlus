 ...
 * <br>
 * 41位时间截(毫秒级)，注意，41位时间截不是存储当前时间的时间截，而是存储时间截的差值（当前时间截 - 开始时间截) 得到的值），这里的的开始时间截，一般是我们的id生成器开始使用的时间，由我们程序来指定的（如下START_TIME属性）。41位的时间截，可以使用69年，年T = (1L << 41) / (1000L * 60 * 60 * 24 * 365) = 69<br>
 * <br>
...
public final class Snowflake {
	 ...
	 * 起始时间戳
	 **/
	private final static long START_TIME = 1329999999999L;
	...
	//单例，非真实单例，只是启动缓存作用
	private static Snowflake one = new Snowflake(1);
	...
	public Snowflake(long dataCenterId) {
		this(dataCenterId, getWorkId(), 5L, false);
	}

	public Snowflake(long dataCenterId, boolean randomSequence) {
		this(dataCenterId, getWorkId(), 5L, randomSequence);
	}
	 ...
	 */
	public Snowflake(long dataCenterId, long workerId, long timeOffset, boolean randomSequence) {
	...
	public synchronized Long nextId() {
		...
		/*
		 * 1.左移运算是为了将数值移动到对应的段(41、5、5，12那段因为本来就在最右，因此不用左移) 2.然后对每个左移后的值(la、lb、lc、sequence)做位或运算，是为了把各个短的数据合并起来，合并成一个二进制数 3.最后转换成10进制，就是最终生成的id
		 */
